#!/usr/bin/env python3
"""Small BrainHub command runner.

Usage:
  python brainhub_engine.py init [target]
  python brainhub_engine.py serve [target]
  python brainhub_engine.py demo [target]
  python brainhub_engine.py try [target]
  python brainhub_engine.py proof [target]
  python brainhub_engine.py onboard [target]
  python brainhub_engine.py seed [project-dir] [target]
  python brainhub_engine.py welcome [target]
  python brainhub_engine.py prompts [target]
  python brainhub_engine.py status [target]
  python brainhub_engine.py health [target]
  python brainhub_engine.py operations [target]
  python brainhub_engine.py backup [target]
  python brainhub_engine.py restore-backup <backup-name-or-path> [target]
  python brainhub_engine.py compliance-export [target]
  python brainhub_engine.py team-sync [target]
  python brainhub_engine.py share <page-or-memory> [target]
  python brainhub_engine.py snapshot [target]
  python brainhub_engine.py doctor [target]
  python brainhub_engine.py migrate [target]
  python brainhub_engine.py validate [target]
  python brainhub_engine.py ingest-status [target]
  python brainhub_engine.py import-obsidian <vault> [target]
  python brainhub_engine.py remember "memory text" [target]
  python brainhub_engine.py propose-memories <file-or-text> [target]
  python brainhub_engine.py session-end <file-or-text> [target]
  python brainhub_engine.py capture-inbox [target]
  python brainhub_engine.py update-memory <name-or-title> "new memory text" [target]
  python brainhub_engine.py query "task or question" [target]
  python brainhub_engine.py graph-summary ["topic"] [target]
  python brainhub_engine.py benchmark ["query"] [target]
  python brainhub_engine.py brief ["task or question"] [target]
  python brainhub_engine.py recall "query" [target]
  python brainhub_engine.py profile [target]
  python brainhub_engine.py wins [target]
  python brainhub_engine.py memory-audit [target]
  python brainhub_engine.py archive-memory <name-or-title> [target]
  python brainhub_engine.py restore-memory <name-or-title> [target]
  python brainhub_engine.py forget-memory <name-or-title> [target] --confirm
  python brainhub_engine.py memory-inbox [target]
  python brainhub_engine.py memory-log [target]
  python brainhub_engine.py review-memory <name-or-title> [target]
  python brainhub_engine.py explain-memory <name-or-title> [target]
  python brainhub_engine.py rebuild-index [target]
  python brainhub_engine.py rebuild-backlinks [target]
  python brainhub_engine.py verify-mcp [target]
  python brainhub_engine.py connect <agent> [target]
"""
...
PROOF_MEMORY_TEXT = (
    "For the BrainHub cross-agent proof, remember that local agent memory should be "
...
from brainhub_core.audit_export import (
...
from brainhub_core.cli_memory import (
...
from brainhub_core.agent_hooks import (
...
from brainhub_core.obsidian import (
...
# Memory-layer verbs refused when the workspace config disables the memory
# layer (brainhub.config.json: "memory_enabled": false). Wiki/document/artifact
# verbs are never gated.
MEMORY_LAYER_COMMANDS = {
    "remember", "propose-memories", "capture-session", "session-end",
    "capture-inbox", "accept-capture", "redact-capture", "delete-capture",
    "update-memory", "set-memory-visibility", "recall", "brief", "start",
    "hook", "consolidate", "semantic", "profile", "wins", "memory-audit",
    "archive-memory", "restore-memory", "forget-memory", "memory-inbox",
    "memory-log", "review-memory", "explain-memory",
}


def _memory_layer_gate(args) -> int | None:
    """Return an exit code when a memory-layer verb hits a memory-disabled workspace."""
    if getattr(args, "command", "") not in MEMORY_LAYER_COMMANDS:
        return None
    root = _resolve_link_root(Path(getattr(args, "target", ".")))
    if _core_memory_layer_enabled(root):
        return None
    print(_core_memory_disabled_notice(root), file=sys.stderr)
    return 2
...
def _capture_review_summary(target: Path, project: str | None = None, limit: int = 3) -> dict[str, object]:
    ...
    summary["next_action"] = f'python3 brainhub_engine.py capture-inbox "{root}"'
    if summary["project"]:
        summary["next_action"] = f'python3 brainhub_engine.py capture-inbox "{root}" --project "{summary["project"]}"'
    return summary
...
def start(
    ...
    payload = {
        ...
        "project_seed": {
            ...
            "safety": (
                "Run from the project repo. BrainHub reads allowlisted project docs/rules, "
...
def semantic(target: Path, setup: bool = False, rebuild: bool = False, json_output: bool = False) -> int:
    ...
    if setup or rebuild:
        ...
        if embedder is None:
            install_hint = f'{sys.executable} -m pip install "brainhub-mcp[semantic]"'
            action_error = (
                f"Semantic provider unavailable for {sys.executable}. Install it first: {install_hint}"
                if setup
                else "Semantic model not available offline. Run: bh semantic --setup"
            )
            mcp_python = _core_resolve_mcp_python(target, wiki_dir, None, default_python=sys.executable)
            if mcp_python != sys.executable:
                action_error += (
                    f"\nYour BrainHub MCP Python is {mcp_python}. If you installed the extra there, "
                    f"set it up through the MCP runtime instead: "
                    f"{mcp_python} -m brainhub_mcp --semantic-setup --wiki {wiki_dir}"
                )
        else:
...
def _hook_session_start(
    ...
    if not wiki_dir.exists():
        _emit_session_start(
            f"BrainHub: wiki missing at {wiki_dir}; run {_display_command(['bh', 'init', str(target)])} to restore it.",
            emit,
        )
        return 0
...
def _session_end_hook_state_path(target: Path) -> Path:
    return _resolve_link_root(target) / ".brainhub-cache" / "session-end-hook.hash"
...
def run_agent_hook(
    ...
    try:
        ...
        print(f"BrainHub {event} hook failed: {exc}", file=sys.stderr)
    return 0
...
def connect_mcp(
    ...
    *,
    ...
    if hooks:
        runtime_script = target / "brainhub_engine.py"
        runtime_note = ""
        if runtime_script.exists():
            # A workspace runtime copied before session hooks existed would
            # make every installed hook fail with an argparse error.
            if not (target / "brainhub_core" / "agent_hooks.py").exists():
                if write:
                    _copy_runtime_files(target)
                    runtime_note = f"Refreshed the BrainHub runtime at {target}: it predated session hooks."
                else:
                    runtime_note = (
                        f"The BrainHub runtime at {target} predates session hooks; "
                        "--write will refresh it automatically (or run "
                        f"{_display_command(['bh', 'init', str(target)])} first)."
                    )
        else:
            runtime_script = ROOT / "brainhub_engine.py"
    ...
    *,
        ...
        try:
            ...
            if hooks and _core_supports_agent_hooks(agent):
                connection["session_hooks"] = _core_build_agent_hooks_payload(
                    target=target,
                    agent=agent,
                    runtime_script=(target / "brainhub_engine.py") if (target / "brainhub_engine.py").exists() else ROOT / "brainhub_engine.py",
                    python_cmd=sys.executable,
                    write=write,
                )
            elif hooks:
    ...
    if first_memory and isinstance(prompts, list):
        prompts = [
            ...
            if isinstance(item, dict) and item.get("label") == "存明確記憶"
            else item
            for item in prompts
        ]
    commands = {
...
def create_demo(target: Path, force: bool = False) -> int:
    ...
    code, text = _core_render_demo_text(
        ...
        serve_command=_display_command(["python3", str(target / "brainhub_engine.py"), "serve", str(target)]),
        next_command=_display_command(["python3", str(target / "brainhub_engine.py"), "next", str(target)]),
        start_command=_display_command([
            "python3",
            str(target / "brainhub_engine.py"),
            "start",
            str(target),
            "--task",
        ...
        audit_command=_display_command(["python3", str(target / "brainhub_engine.py"), "memory-audit", str(target)]),
    )
    _print_text(text)
    return code
...
def proof(
    ...
    *,
    ...
    if memory_result.get("created") and memory_result.get("name"):
        try:
            review_result = _mark_memory_reviewed(
                ...
                note="Reviewed automatically because the user explicitly ran bh proof.",
            )
    ...
    payload = {
        ...
        "prompts": {
            "agent_a": "remember that I want BrainHub memory shared across my local agents",
