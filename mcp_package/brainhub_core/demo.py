"""Bundled first-run demo wiki content for BrainHub."""
...
RUNTIME_FILES = ("serve.py", "brainhub_engine.py", "BRAINHUB-SCHEMA.md", ".brainhubignore")
BRAND_FILES = ("logo.png", "logo.svg")
WORKSPACE_GITIGNORE = """# BrainHub private local source material
raw/*
!raw/.gitkeep

# BrainHub local runtime state
.brainhub-backups/
.brainhub-cache/
.brainhub-mcp-python
...
def copy_runtime_files(source_root: Path, target: Path) -> None:
    """Copy the runtime files needed by a standalone BrainHub wiki directory."""
    ...
    core_src = source_root / "mcp_package" / "brainhub_core"
    if not core_src.exists():
        core_src = source_root / "brainhub_core"
    if core_src.exists():
        core_target = target / "brainhub_core"
...
def create_demo_workspace(target: Path, *, source_root: Path, force: bool = False) -> dict[str, object]:
    ...
    if target.exists() and any(target.iterdir()):
        ...
        if not force:
            raise DemoError(f"{target} already exists. Re-run with --force to replace a BrainHub demo directory.")
        if not marker.exists():
            raise DemoError(f"{target} does not look like a BrainHub demo directory; refusing to overwrite it.")
        shutil.rmtree(target)

    target.mkdir(parents=True, exist_ok=True)
    atomic_write_text(target / DEMO_MARKER, "BrainHub demo directory\n")
...
DEMO_FILES: dict[str, str] = {
    "START_HERE.md": """# BrainHub Demo: Start Here
...
```text
is BrainHub ready?
query BrainHub for why BrainHub helps agents
start with BrainHub before we continue
what does BrainHub remember about local personal memory?
explain why BrainHub remembers local personal memory
```

## Try These CLI Checks

```bash
python3 brainhub_engine.py query "why does BrainHub help agents?" . --budget small
python3 brainhub_engine.py brief "working on agent memory" .
python3 brainhub_engine.py memory-audit .
python3 brainhub_engine.py health .
```
...
```bash
python3 brainhub_engine.py serve .
```
...
> **TLDR:** A demo release note showing BrainHub as a local wiki viewer, graph, and MCP memory server.
...
- **BrainHub exposes search, context, backlinks, and graph tools.** `[confidence: high]`
...
Agent memory addresses a common failure mode in AI workflows: each new session starts without the full project history. In BrainHub, memory is stored as markdown wiki pages compiled from immutable raw sources. *Source: [[agent-memory-session]]* `[confidence: high]`
...
3. BrainHub builds search indexes and backlinks.
...
In BrainHub, the retrieval layer is a local markdown wiki exposed through search, context, backlinks, and graph tools. This makes [[agent-memory]] inspectable instead of hidden in a proprietary store. *Source: [[transformer-reading-notes]]* `[confidence: high]`
...
BrainHub follows this model by storing raw sources and wiki pages as markdown files. The graph and MCP server read those files rather than sending them to a hosted backend. *Source: [[local-release-notes]]* `[confidence: high]`
...
In BrainHub, the knowledge graph makes relationships between sources, concepts, and entities visible. This helps users inspect what an agent has connected and where a claim came from. *Source: [[local-release-notes]]* `[confidence: high]`
...
title: "BrainHub"
entity_type: project
aliases: ["BrainHub wiki", "BrainHub MCP", "BrainHub"]
date_created: "2026-05-02"
date_updated: "2026-05-02"
...
> **TLDR:** BrainHub is a local-first wiki and MCP server that turns source notes into durable memory for AI agents.

## Overview

BrainHub stores source material in `raw/` and compiled wiki pages in `wiki/`. The web viewer makes the wiki readable by humans, while MCP tools make the same knowledge readable by agents. *Source: [[local-release-notes]]* `[confidence: high]`

The demo positions BrainHub as a local [[agent-memory]] layer. It keeps knowledge inspectable through markdown and navigable through a [[knowledge-graph]]. *Source: [[agent-memory-session]]* `[confidence: high]`
...
> **TLDR:** The user wants BrainHub to be local personal memory for agents, with the wiki as the inspectable storage format.
...
- Positioning BrainHub in product copy or onboarding.
...
When answering from BrainHub, keep responses short and point to the [[knowledge-graph]] pages behind each claim instead of repeating whole documents.

## Use This When

- Formatting any answer that uses BrainHub context.
...
> **TLDR:** The local BrainHub viewer serves this wiki at http://127.0.0.1:3000 by default.

## Memory

`bh serve` binds the read-only local viewer for [[link]] to `127.0.0.1:3000`. It is loopback-only and needs no account or network access.
...
""",
    "wiki/explorations/why-brainhub-helps-agents.md": """---
type: exploration
title: "Why BrainHub helps agents"
date_created: "2026-05-02"
query: "Why does BrainHub help AI agents?"
...
BrainHub helps agents because it turns past project material into durable, queryable context. Instead of starting each session from a blank chat, an agent can ask for [[agent-memory]] and receive the main page plus related concepts, sources, and entities.
...
The answer combines [[agent-memory-session]], [[transformer-reading-notes]], and [[local-release-notes]]. Together they show BrainHub as a local retrieval layer for AI workflows: sources become pages, pages form a [[knowledge-graph]], and MCP exposes that graph to agents.
...
- [[retrieval-augmented-generation]]
""",
    "wiki/index.md": """# BrainHub Demo Wiki Index
...
### entities
- [[link]] - Local-first wiki and MCP memory server for agents. growing - 2 sources - also: BrainHub MCP

### memories
- [[prefer-local-personal-memory]] - User preference that BrainHub should behave as local personal memory for agents. preference · user
- [[keep-answers-short-and-cite-wiki-sources]] - Reviewed style preference for answers built from BrainHub context. preference · user
...
- [[local-release-notes]] - Demo note on BrainHub surfaces and graph visibility. high

### explorations
- [[why-brainhub-helps-agents]] - Filed answer explaining BrainHub as durable agent memory.
...
""",
    "wiki/log.md": """# BrainHub Demo Wiki Log
...
- Created: explorations/why-brainhub-helps-agents.md
