"""Local permalink helpers for BrainHub wiki pages."""
...
def share_page_payload(
    ...
    *,
    ...
    try:
        ...
        serve_command = ["bh", "serve", str(root), "--port", str(port)]
        return {
            "found": True,
            "query": query,
            ...
            "agent_prompt": f"open BrainHub page {page_name} and summarize why it matters",
            "candidates": candidates,
        }
    finally:
        if owns_cache:
...
def render_share_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    if not payload.get("found"):
        lines = [
            "BrainHub share: no matching page",
            "",
