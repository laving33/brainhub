"""Lightweight operation journal for BrainHub's local multi-file writes."""
...
OPERATION_DIR_NAME = ".brainhub-operations"
...
def _slug(value: str, fallback: str = "operation") -> str:
    return slugify(value, fallback=fallback, max_len=80)
...
def _operation_marker(wiki_dir: Path, marker_name: str) -> Path:
    ...
    if root != resolved.parent:
        raise ValueError("operation marker must stay inside wiki/.brainhub-operations")
...
def operation_report(
    ...
    *,
    ...
    """Return a user-facing summary of interrupted or active BrainHub write operations."""
    wiki_dir = wiki_dir.expanduser().resolve()
        ...
        next_actions.extend([
            {
                ...
                "command": display_command(["bh", "operations", command_target]),
            },
            {
                "label": "validate wiki structure after reviewing interrupted writes",
                "command": display_command(["bh", "validate", command_target]),
            },
            {
                "label": "repair generated indexes if validation reports stale graph data",
                "command": display_command(["bh", "doctor", "--fix", command_target]),
            },
        ])
    elif active_count:
        next_actions.append({
            "label": "wait for the active BrainHub write to finish, then rerun this command",
            "command": display_command(["bh", "operations", command_target]),
        })
    else:
        next_actions.append({
            "label": "continue using BrainHub normally",
            "command": display_command(["bh", "status", "--validate", command_target]),
        })
    return {
        "wiki": str(wiki_dir),
...
def render_operation_recovery_text(payload: dict[str, object], *, target: object = ".") -> tuple[int, str]:
    ...
    lines = [f"BrainHub operation recovery: {payload.get('marker') or 'unknown'}", ""]
    ...
    if payload.get("requires_confirm"):
        lines.extend([
            ...
            f"  {display_command(['bh', 'operations', str(target), '--recover', str(payload.get('marker') or ''), '--confirm'])}",
            "",
            "Result: preview",
        ])
        return 1, "\n".join(lines)
...
def render_operations_text(payload: dict[str, object]) -> tuple[int, str]:
    ...
    lines = [f"BrainHub operations: {payload.get('wiki')}", ""]
    if not operations:
        lines.append("No pending, failed, or interrupted BrainHub operations.")
    else:
