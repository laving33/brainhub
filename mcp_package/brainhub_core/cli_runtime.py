"""Text rendering helpers for BrainHub setup-oriented CLI commands."""
...
def render_init_text(*, target: object, fixes: Sequence[str]) -> tuple[int, str]:
    ...
    lines = [f"BrainHub wiki ready at {target}"]
    if fixes:
    ...
    lines.extend([
        "",
        "Next:",
        f"  {display_command(['bh', 'health', command_target])}",
        f"  {display_command(['bh', 'onboard', command_target])}",
        f"  {display_command(['bh', 'serve', command_target])}",
        "  Open http://127.0.0.1:3000/onboard for the browser checklist",
        "  Drop sources into raw/ and ask your agent: ingest raw/<file> into BrainHub",
    ])
    return 0, "\n".join(lines)


def render_starter_prompts_text(payload: Mapping[str, object]) -> tuple[int, str]:
    lines = [f"BrainHub starter prompts: {payload['target']}"]
    if payload["project"]:
...
def render_start_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    if not isinstance(status, Mapping):
        raise ValueError("Invalid BrainHub start payload")
    ...
    if not status.get("ready"):
        ...
        if isinstance(next_actions, Sequence) and not isinstance(next_actions, (str, bytes)):
            for item in next_actions[:3]:
                if isinstance(item, Mapping):
                    lines.append(f"- {item.get('label', item.get('tool', 'inspect BrainHub'))}")
...
def render_welcome_text(payload: Mapping[str, object]) -> tuple[int, str]:
    """Render a short first-use guide for humans trying BrainHub with an agent."""
    lines = [f"BrainHub welcome: {payload['target']}"]
    if payload["project"]:
        lines.append(f"Project: {payload['project']}")
    lines.extend([
        "",
...
def render_demo_text(
    *,
    ...
    return 0, "\n".join([
        f"BrainHub demo created at {target}",
        "",
        "View it:",
        f"  {serve_command}",
        "",
...
def render_proof_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    lines = [
        ...
        f"Agent B: {prompts.get('agent_b', 'start with BrainHub before we continue')}",
        "",
        "No viewer required",
...
def render_onboard_text(payload: Mapping[str, object]) -> tuple[int, str]:
    """Render the guided first-run path for a normal BrainHub workspace."""
    ...
    lines = [
        f"BrainHub onboard: {payload.get('target')}",
        "",
        "Workspace",
...
def render_mcp_connect_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    lines = [
        f"BrainHub connect: {payload.get('display_name')}",
        "",
...
def render_agent_hooks_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    code = 0 if not requested or ok else 1
    lines = [
        f"BrainHub session hooks: {payload.get('display_name')}",
        "",
...
def render_session_start_hook_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    lines = [
        "BrainHub memory (local, source-backed)"
        + (f" · project {project}" if project else ""),
    ]
    if not status.get("ready"):
        lines.extend([
            "BrainHub is not ready; skipping the memory brief.",
            f"Check with: {display_command(['bh', 'health', target])}",
        ])
        return 0, "\n".join(lines)
    ...
    if (
        ...
        lines.extend([
            ...
            f"{display_command(['bh', 'seed', '.', target])} (source-backed, no durable memory).",
    ...
    if seed_recommended:
        lines.extend([
            ...
            f"from this repo's docs, suggest: {display_command(['bh', 'seed', '.', target])}",
        ])
    ...
    lines.extend([
        ...
        f"For task-specific context: {display_command(['bh', 'query', '<topic>', target, '--budget', 'micro'])} "
        "or the BrainHub MCP recall tool. Save durable memory only after explicit user approval.",
    ])
    return 0, "\n".join(lines)
