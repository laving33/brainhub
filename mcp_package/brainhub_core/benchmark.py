"""Shared benchmark health helpers for BrainHub."""
...
def benchmark_value_evidence(cache: Mapping[str, object], packet: Mapping[str, object]) -> dict[str, object]:
    ...
    the bounded BrainHub query packet against the body text currently indexed in
    the local wiki.
    """
    ...
    notes = [
        ...
        "This does not score answer quality; use it to see whether BrainHub is reducing context budget waste.",
    ]
    if broad_chars == 0:
...
def build_benchmark_payload(
    ...
    *,
    ...
    """Build benchmark timings and scale metadata for a BrainHub wiki."""
...
def benchmark_health(payload: Mapping[str, object]) -> dict[str, object]:
    ...
    if warnings:
        ...
        recommendations = [
            "Run bh doctor --fix and bh benchmark again after repairing wiki/index state.",
        ]
...
def render_benchmark_text(payload: Mapping[str, object]) -> str:
    ...
    lines = [
        f"BrainHub benchmark: {payload.get('target', '')}",
