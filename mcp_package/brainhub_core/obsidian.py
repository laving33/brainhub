"""Obsidian vault import helpers for BrainHub."""
...
def import_obsidian_vault(
    ...
    *,
    ...
    """Copy Obsidian Markdown notes into ``raw/obsidian/<vault>/`` for ingest."""
    ...
    raw_prefix = root / "raw" / "obsidian" / vault_slug
    ...
    next_commands = [f"bh ingest-status {root}", f"bh validate {root}", f"bh health {root}"]
    if not has_wiki_dir:
        next_commands = [f"bh init {root}", *next_commands]
    return {
        "status": status,
        "target": str(root),
        ...
        "next_prompt": f"ingest raw/obsidian/{vault_slug} into BrainHub",
...
def render_import_obsidian_text(payload: dict[str, object]) -> tuple[int, str]:
    ...
    if not payload.get("has_wiki_dir"):
        lines.append("BrainHub wiki is not initialized yet; run the init command below before ingest.")
    if payload.get("imported"):
        lines.append("")
        lines.append("Imported raw files:")
