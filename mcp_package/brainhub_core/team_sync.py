"""Read-only Git team-sync guidance for BrainHub workspaces."""
...
def _link_root(target: Path) -> Path:
    ...
    if root.name == "wiki" and (root / "_brainhub_schema.json").exists():
        return root.parent
    return root
...
def _shared_git_paths(root: Path, *, absolute: bool) -> list[str]:
    """Return share-safe BrainHub paths for Git commands.
...
def build_team_sync_payload(target: Path, *, remote: str | None = None) -> dict[str, object]:
    """Return a read-only plan for sharing a BrainHub workspace through Git."""
    ...
    warnings: list[str] = []
    if not wiki_dir.exists():
        warnings.append("BrainHub wiki is missing. Run bh init before preparing team sync.")
    ...
    sync_actions: list[dict[str, str]] = [
        _action("check BrainHub health", ["bh", "health", str(root)]),
        _action("review pending memories", ["bh", "memory-inbox", str(root)]),
        _action("validate before sharing", ["bh", "validate", str(root)]),
        _action("backup before sharing", ["bh", "backup", str(root)]),
    ]
    if git_root is None:
        setup_actions.extend([
            ...
            _action("commit shared memory baseline", ["git", "-C", str(root), "commit", "-m", "Initialize BrainHub shared memory"]),
        ])
        if remote_clean:
        ...
        sync_actions.extend([
            ...
            _action("commit reviewed memory updates", ["git", "-C", str(git_root), "commit", "-m", "Update BrainHub shared memory"]),
        ])
    ...
    return {
        ...
        "notes": [
            "Share wiki/ and BRAINHUB-SCHEMA.md for team agent memory.",
...
def render_team_sync_text(payload: Mapping[str, object]) -> tuple[int, str]:
    ...
    lines = [
        f"BrainHub team sync: {payload.get('target')}",
        "",
