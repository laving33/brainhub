"""Shared BrainHub runtime status helpers."""
from __future__ import annotations

from pathlib import Path
from typing import Any, Iterable, Mapping

from .config import memory_layer_enabled
...
def link_status(
    ...
    *,
        ...
        try:
            ...
            if stale_operations:
                warnings.append({
                    ...
                    "message": f"{len(stale_operations)} incomplete BrainHub operation(s) need review.",
                    "detail": str([
                        {
                ...
                warnings.append({
                    ...
                    "message": f"{len(fresh_operations)} BrainHub operation(s) are currently in progress.",
                    "detail": str([
                        {
                            "operation": item.get("operation"),
                            "description": item.get("description"),
            ...
            warnings.append(_warning(
                ...
                "Could not inspect BrainHub operation markers.",
                exc,
            ))
        ...
        try:
            ...
            if pages and search_backend != "sqlite-fts":
                warnings.append({
                    ...
                    "message": "SQLite FTS search is not active; BrainHub is using the slower token-index fallback.",
    ...
    if missing:
        next_actions.append(_action("repair or scaffold BrainHub structure", "doctor", {"fix": True}))
    if schema.get("status") in {"missing", "old"}:
        next_actions.append(_action("write current BrainHub wiki schema marker", "admin", {"action": "migrate"}))
    elif schema.get("status") == "invalid":
        next_actions.append(_action("inspect invalid BrainHub wiki schema marker", "doctor"))
    elif schema.get("status") == "newer":
        next_actions.append(_action("upgrade BrainHub before writing this wiki", "upgrade_link"))
    ...
    if stale_operation_degraded:
        next_actions.append(_action("inspect interrupted BrainHub operation markers", "doctor"))
    ...
    if ready and content_page_count:
        if memory_layer_enabled(wiki_dir.parent):
            ...
            # Memory layer disabled: never point at recall/remember; the
            # document layer is the read surface for this workspace.
            next_actions.append(_action("search wiki documents for the task", "bh_search", {"query": "<user task>"}))
    elif ready:
