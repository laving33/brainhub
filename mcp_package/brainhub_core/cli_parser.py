"""Argument parser for the BrainHub command-line interface."""
...
def build_cli_parser(
    ...
    init_cmd = sub.add_parser("init", help="create or repair a normal BrainHub wiki")
    init_cmd.add_argument("target", nargs="?", default=".")

    serve_cmd = sub.add_parser("serve", help="start the local BrainHub web viewer")
    serve_cmd.add_argument("target", nargs="?", default=".")
    serve_cmd.add_argument("--port", type=int, default=3000)

    demo = sub.add_parser("demo", help="create a pre-ingested sample BrainHub wiki")
    demo.add_argument("target", nargs="?", default=default_demo_dir)
    demo.add_argument("--force", action="store_true", help="replace an existing BrainHub demo directory")
    ...
    try_cmd.add_argument("--force", action="store_true", help="replace an existing BrainHub demo directory")
    ...
    proof_cmd.add_argument("--force", action="store_true", help="replace an existing BrainHub proof workspace")
    ...
    welcome_cmd = sub.add_parser("welcome", help="print the shortest first-use path for BrainHub")
    ...
    status_cmd = sub.add_parser("status", help="show BrainHub readiness, counts, and next actions")
    ...
    operations_cmd = sub.add_parser("operations", help="inspect interrupted or active BrainHub write operations")
    ...
    restore_backup_cmd = sub.add_parser("restore-backup", help="preview or restore a local BrainHub backup archive")
    restore_backup_cmd.add_argument("backup", help="backup filename from .brainhub-backups/ or path to a .tar.gz archive")
    ...
    team_sync_cmd = sub.add_parser("team-sync", help="print a safe Git plan for sharing reviewed BrainHub memory")
    ...
    snapshot_cmd.add_argument("--title", default="BrainHub", help="snapshot title")
    snapshot_cmd.add_argument("--json", action="store_true", help="print machine-readable snapshot status")

    doctor_cmd = sub.add_parser("doctor", help="check a BrainHub wiki for common health issues")
    ...
    migrate_cmd = sub.add_parser("migrate", help="apply safe BrainHub wiki schema migrations")
    ...
    obsidian_cmd = sub.add_parser("import-obsidian", help="copy Obsidian Markdown notes into raw/ for BrainHub ingest")
    ...
    query_cmd = sub.add_parser("query", aliases=["query-link"], help="build a compact answer-ready BrainHub context packet")
    ...
    start_cmd = sub.add_parser("start", help="start a session with BrainHub readiness and a memory brief")
    ...
    profile_cmd = sub.add_parser("profile", help="show what BrainHub remembers")
    ...
    wins_cmd = sub.add_parser("wins", help="show local proof signals for what BrainHub memory is carrying")
    ...
    verify_mcp_cmd = sub.add_parser("verify-mcp", help="verify brainhub-mcp import and print MCP config")
    ...
    connect_cmd.add_argument(
        "--hooks",
        action="store_true",
        help="also configure session hooks so new sessions start with the BrainHub brief (Claude Code)",
    )
...
def dispatch_cli_command(args: Any, handlers: Mapping[str, CliHandler]) -> int:
    """Dispatch parsed BrainHub CLI arguments to runtime-provided handlers."""
