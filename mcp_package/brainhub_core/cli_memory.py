"""Text rendering helpers for BrainHub memory CLI commands."""
...
def render_remember_text(result: Mapping[str, object], *, target: object = ".") -> tuple[int, str]:
    if not result.get("created"):
        if result.get("conflict"):
            ...
            if first:
                lines.append(f"  {_shell_words('python3', 'brainhub_engine.py', 'explain-memory', first, target)}")
    ...
    lines.extend([
        "",
        "Next:",
        f"  {_shell_words('python3', 'brainhub_engine.py', 'recall', result['title'], target)}",
    ])
    return 0, "\n".join(lines)
...
def render_update_memory_text(result: Mapping[str, object], *, target: object = ".") -> tuple[int, str]:
    ...
    return 0, "\n".join([
        ...
        "",
        "Next:",
        f"  {_shell_words('python3', 'brainhub_engine.py', 'explain-memory', result['name'], target)}",
        f"  {_shell_words('python3', 'brainhub_engine.py', 'review-memory', result['name'], target)}",
    ])
...
def render_set_memory_visibility_text(result: Mapping[str, object], *, target: object = ".") -> tuple[int, str]:
    ...
    return 0, "\n".join([
        ...
        "Next:",
        f"  {_shell_words('python3', 'brainhub_engine.py', 'explain-memory', result['name'], target)}",
        f"  {_shell_words('python3', 'brainhub_engine.py', 'team-sync', target)}",
    ])
...
def render_recall_text(
    *,
    ...
    lines = [f"BrainHub memory recall: {query}"]
    if project:
    ...
    if not results:
        ...
        lines.extend([
            "",
            "Next:",
            f"  Add one: {_shell_words('python3', 'brainhub_engine.py', 'remember', 'Memory to keep', target)}",
        ])
        return 0, "\n".join(lines)
...
def render_memory_status_text(result: Mapping[str, object], *, action: str, target: object = ".") -> tuple[int, str]:
    if action == "archive":
        ...
        next_lines = [
            "",
            "Next:",
            f"  Restore: {_shell_words('python3', 'brainhub_engine.py', 'restore-memory', result['name'], target)}",
        ]
    elif action == "restore":
...
def render_forget_memory_text(result: Mapping[str, object], *, identifier: str, target: object = ".") -> tuple[int, str]:
    ...
    if result.get("confirmation_required"):
        return 1, "\n".join([
            ...
            f"Run: {_shell_words('python3', 'brainhub_engine.py', 'forget-memory', result['name'], target, '--confirm')}",
        ])
    return 0, "\n".join([
        "Memory forgotten",
        f"Title: {result['title']}",
...
def render_memory_inbox_text(
    ...
    *,
    ...
    lines = [f"BrainHub memory inbox: {target}"]
...
def render_memory_log_text(payload: Mapping[str, object], *, target: object) -> tuple[int, str]:
    ...
    if not isinstance(entries, Sequence) or isinstance(entries, (str, bytes)) or not entries:
        lines.extend([
            ...
            "",
            "Next:",
            f"  {_shell_words('python3', 'brainhub_engine.py', 'remember', 'a useful preference or decision', target)}",
        ])
        return 0, "\n".join(lines)
...
def render_explain_memory_text(explanation: Mapping[str, object]) -> tuple[int, str]:
    ...
    lines = [
        f"BrainHub memory explanation: {memory['title']}",
        "",
        f"Path: {memory['path']}",
...
def render_profile_text(
    ...
    *,
    ...
    lines = [f"BrainHub memory profile: {target}"]
    if project:
        lines.append(f"Project: {project}")
    lines.append("")
...
def render_memory_audit_text(payload: Mapping[str, object], *, target: object) -> tuple[int, str]:
    lines = [f"BrainHub memory audit: {target}"]
    if payload["project"]:
