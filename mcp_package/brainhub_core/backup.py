"""Local backup helpers for BrainHub wiki data."""
...
BACKUP_DIR_NAME = ".brainhub-backups"
...
def _safe_label(label: str) -> str:
    return slugify(label, fallback="manual")
...
def create_backup(
    ...
    *,
    ...
    """Create a timestamped local backup archive for a BrainHub root.
    ...
    if not wiki_dir.exists() or not wiki_dir.is_dir():
        raise FileNotFoundError(f"BrainHub wiki not found at {wiki_dir}")
...
def list_backups(link_root: Path, *, limit: int = 20) -> dict[str, Any]:
    """Return recent local backups for a BrainHub root."""
    root = link_root.expanduser().resolve()
    backup_dir = root / BACKUP_DIR_NAME
...
def restore_backup(
    ...
    *,
    ...
    """Restore wiki/ and optionally raw/ from a local BrainHub backup archive."""
    root = link_root.expanduser().resolve()
