"""Shared BrainHub ingest status helpers."""
...
def _source_page_suggestion(raw_rel: str) -> str:
    ...
    slug = slugify(stem, fallback="source")
...
def _target_command(command: str, target: str) -> str:
    if target and target in command:
        return command
    parts = command.split()
    if not parts or parts[0] not in {"link", "bh"} or not target:
        return command
...
def build_ingest_plan(status: dict[str, object], limit: int = 5) -> dict[str, object]:
    ...
    if state == "blocked_secrets":
        ...
        return {
            ...
            "summary": f"Start with {first['raw']}; BrainHub will not suggest ingesting secret-looking raw content.",
            "batch": [
                {
            ...
            "agent_prompt": None,
            "memory_prompt": None,
            "post_checks": ["bh ingest-status", "bh health"],
        }
    ...
    if state == "blocked_raw_access":
        ...
        return {
            ...
            "summary": f"Start with {first['raw']}; BrainHub could not read it to run safety checks.",
            "batch": [
                {
                    "raw": str(item.get("raw") or ""),
                    "size_bytes": int(item.get("size_bytes") or 0),
            ...
            "steps": [
                ...
                "Only ingest after BrainHub can inspect the raw source for secret-looking values.",
            ],
            "agent_prompt": None,
            "memory_prompt": None,
            "post_checks": ["bh ingest-status", "bh health"],
    ...
    if state == "blocked_source_access":
        ...
        return {
            ...
            "summary": f"Start with {first['page']}; BrainHub could not read one or more source pages.",
            "batch": [
                {
            ...
            "agent_prompt": None,
            "memory_prompt": None,
            "post_checks": ["bh ingest-status", "bh validate", "bh health"],
        }
    ...
    if state == "stale_graph":
        return {
            ...
            "agent_prompt": guidance.get("agent_prompt"),
            "post_checks": ["bh rebuild-backlinks", "bh validate", "bh health"],
        }
    ...
    if state == "ready":
        return {
            ...
            "steps": [
                ...
                "Add new files to raw/ when BrainHub should learn new source-backed context.",
            ],
            "agent_prompt": None,
            "post_checks": ["bh doctor", "bh health"],
        }

    return {
        "state": state,
        "title": "Initialize BrainHub",
        "summary": "BrainHub needs its raw/ and wiki/ structure before ingest can start.",
        "batch": [],
        "steps": [
            "Run bh init or rerun an installer.",
            "Check readiness before adding sources.",
        ],
        "agent_prompt": None,
        "post_checks": ["bh init", "bh health"],
    }
...
def build_ingest_completion(status: dict[str, object], limit: int = 8) -> dict[str, object]:
    ...
    for item in represented_raw[: max(limit, 1)]:
        ...
        items.append({
            ...
            "query_prompt": f"query BrainHub for {Path(raw_rel).stem.replace('-', ' ')}" if raw_rel else "",
            "secret_warnings": list(item.get("secret_warnings") or []),
            "scan_error": str(item.get("scan_error") or ""),
        })
    ...
    if represented_count and pending_count:
        ...
        next_prompt = 'start with BrainHub before we continue'
    else:
        summary = "No raw source files are represented yet."
        next_prompt = str(guidance.get("agent_prompt") or "ingest raw/<file> into BrainHub")
...
def render_ingest_status_text(target: str, status: dict[str, object]) -> str:
    ...
    lines = [f"BrainHub ingest status: {target}", ""]
    ...
    if not status["has_raw_dir"] or not status["has_wiki_dir"]:
        lines.extend(["", "Next:", f"  Run an installer or initialize this directory: {_target_command('bh init', target)}"])
        return "\n".join(lines)
...
def build_ingest_guidance(status: dict[str, object]) -> dict[str, object]:
    ...
    if not has_raw_dir or not has_wiki_dir:
        return {
            ...
            "summary": "BrainHub is not initialized here yet.",
            "agent_prompt": None,
            "commands": ["bh init", "bh health"],
    ...
    if source_read_warning_count:
        return {
            ...
            "agent_prompt": None,
            "commands": ["bh ingest-status", "bh validate", "bh health"],
            "notes": [
    ...
    if access_items:
        ...
        return {
            ...
            "agent_prompt": None,
            "commands": ["bh ingest-status", "bh health"],
            "notes": [
                "Do not ask an agent to ingest raw files that BrainHub cannot read and scan for secret-looking values.",
    ...
    if stale_items:
        ...
        return {
            "state": "stale_raw",
            "summary": summary,
            "agent_prompt": f"re-ingest {first} into BrainHub",
            "commands": ["bh rebuild-index", "bh rebuild-backlinks", "bh validate", "bh health"],
            "notes": [
    ...
    if pending_items:
        ...
        return {
            "state": "pending_raw",
            "summary": summary,
            "agent_prompt": f"ingest {first} into BrainHub",
            "commands": ["bh rebuild-index", "bh rebuild-backlinks", "bh validate", "bh health"],
            "notes": [
    ...
    if backlinks_status != "current":
        return {
            ...
            "agent_prompt": "rebuild BrainHub backlinks and validate the wiki",
            "commands": ["bh rebuild-backlinks", "bh validate", "bh doctor"],
    ...
    if raw_count == 0:
        return {
            "state": "empty",
            "summary": "BrainHub is ready, but raw/ has no source files yet.",
            "agent_prompt": None,
            "commands": ["bh health", "bh serve"],
            "notes": ["Drop notes, articles, transcripts, or project files into raw/, then ask your agent to ingest them into BrainHub."],
        }
    ...
    return {
        ...
        "agent_prompt": None,
        "commands": ["bh doctor", "bh health"],
        "notes": ["Add new files to raw/ when you want BrainHub to learn new source-backed knowledge."],
    }
