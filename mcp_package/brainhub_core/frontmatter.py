"""Shared frontmatter parsing and formatting helpers for BrainHub."""
...
def frontmatter_string(value: object) -> str:
    # Single-line quoted scalar. Escape backslash + quote, and neutralize
    # CR/LF so a caller-controlled value (source/title/...) cannot inject extra
    # frontmatter lines that a later last-key-wins re-parse would honor
    # (e.g. an injected `visibility: team` overriding an intended `private`,
    # which would escape team_sync's private-only git gate = egress).
    return (
        str(value)
        .replace("\\", "\\\\")
        .replace('"', '\\"')
