"""Shared wiki indexing, search, context, and graph helpers for BrainHub."""
...
# v3: the token/word indexes below are built with normalized_search_text(), whose
# tokenization changed (NFKC + all-scripts + CJK bigrams). A v2 cache on disk is
# keyed on file mtimes, which have NOT changed — so without this bump every
# existing brain would happily reuse a CJK-blind index and the fix would be
# invisible in production while passing every test.
...
def _persistent_cache_path(wiki_dir: Path) -> Path:
    return wiki_dir.parent / ".brainhub-cache" / f"wiki-cache-v{PERSISTENT_CACHE_SCHEMA_VERSION}.json"


def _persistent_fts_path(wiki_dir: Path) -> Path:
    # v2: bodies are now bigram-expanded before indexing (search.fts_text), so a
    # v1 sidecar holds tokens this build can no longer query. Same reason as the
    # cache bump above — signatures are mtime-based and would call v1 "fresh".
    return wiki_dir.parent / ".brainhub-cache" / "page-fts-v2.sqlite"
...
def build_wiki_cache(wiki_dir: Path, *, use_persistent_cache: bool = True) -> dict[str, Any]:
    ...
    for record in records:
        ...
        if sid and sid.lower() not in aliases:
            # A sid resolves everywhere an alias resolves: [[sid]] wikilinks,
            # page_index lookups, bh_link targets, viewer /page/<sid>.
            aliases.append(sid.lower())
        ...
        # Index the same units the query is decomposed into (search.text). A raw
        # \W+ split stores an unsegmented Chinese run as ONE key ("客戶資料要怎麼隔離"),
        # which no query can ever hit — the lookup silently misses and search falls
        # through to scanning the whole corpus. Bigrams make the index usable again.
        for token in search_words(text_lower):
            token_index.setdefault(token, set()).add(stem)

        meta_tokens: set[str] = set()
        meta_tokens |= search_words(title)
        for alias in aliases:
            meta_tokens |= search_words(alias)
        for tag in tags_raw:
            meta_tokens |= search_words(str(tag))
        if tldr:
            meta_tokens |= search_words(tldr)
...
def build_index_markdown(
    ...
    """Build a deterministic, human-readable catalog for a BrainHub wiki."""
