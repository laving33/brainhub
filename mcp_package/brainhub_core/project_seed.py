"""Project seeding helpers for day-one BrainHub context."""
...
def discover_project_seed_files(project_root: Path, *, limit: int = DEFAULT_FILE_LIMIT) -> list[dict[str, object]]:
    """Return project files BrainHub can safely consider for a day-one seed."""
    root = project_root.expanduser().resolve()
...
def _format_raw_seed(
    *,
    ...
    parts = [
        ...
        "",
        "This source was generated by `bh seed` from allowlisted project context files.",
...
def _format_source_page(
    *,
    ...
    return "\n".join([
        ...
        "## Summary",
        "",
        f"{summary}{git_note} This page exists so the first BrainHub recall can return useful project context immediately.",
        "",
        "## Key Context",
        "",
        *file_lines,
...
def _ensure_wiki_structure(root: Path) -> None:
    ...
    if not log_path.exists():
        atomic_write_text(log_path, "# BrainHub Wiki Log\n\n")
...
def seed_project_context(
    ...
    *,
    ...
    """Seed BrainHub with source-backed context from common project files."""
    ...
    slug = _slugify(title)
    raw_rel = f"raw/project-seeds/{slug}/project-context.md"
    ...
    next_commands = [
        f"bh query \"what is this project about?\" {root}",
        f"bh brief \"working on {title}\" {root}",
        f"bh health {root}",
    ]
    if status == "already_seeded":
        next_commands.insert(0, f"bh seed {project} {root} --overwrite")
    if status == "needs_attention":
        next_commands = [f"redact blocked project files, then rerun: bh seed {project} {root}"]
    elif status == "empty":
    ...
    return {
        ...
        "next_prompt": f"query BrainHub for {title} project context",
...
def render_seed_project_text(payload: dict[str, object]) -> tuple[int, str]:
    ...
    lines = [
        f"BrainHub project seed: {payload['project_root']}",
