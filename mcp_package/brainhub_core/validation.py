"""Wiki validation helpers for BrainHub ingest gates."""
...
# Optional category dirs: recognized when present but NOT required to exist.
# The document layer (wiki/documents/, type: document) is published on demand;
# keeping it out of TYPE_DIRECTORIES means existing workspaces without documents
# are not flagged as missing a required directory.
OPTIONAL_TYPE_DIRECTORIES = {
    "documents": "document",
}
...
def validate_wiki(wiki_dir: Path, *, strict: bool = False) -> dict[str, Any]:
    """Validate BrainHub wiki structure after agent writes or ingest."""
    ...
    deferred_link_checks: list[tuple[str, str]] = []
    ...
    for page in pages:
        ...
        page_sid = normalize_sid(meta.get("sid"))
        if page_sid:
            page_sids.add(page_sid.lower())
        top_dir = rel.split("/", 1)[0]
        expected_type = TYPE_DIRECTORIES.get(top_dir) or OPTIONAL_TYPE_DIRECTORIES.get(top_dir)
        ...
        if expected_type is None:
            findings.append(_finding("error", "invalid_directory", rel, "Wiki page is outside a known BrainHub category directory."))
            continue
        ...
        for match in WIKILINK_RE.finditer(body):
            ...
            if target and target not in stems:
                # A [[sid]] target is only known once every page's frontmatter
                # has been read; defer the verdict until after the loop.
                deferred_link_checks.append((rel, target))

    for rel, target in deferred_link_checks:
        if target not in page_sids:
