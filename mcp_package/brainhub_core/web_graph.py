...
def graph_initial_payload(
    ...
    if total_node_count > full_node_limit and summary_graph is not None:
        ...
        graph_mode = "summary"
        graph_note = (
            f" 先顯示 {len(visible_nodes)} 個高訊號節點的快速總覽；"
            f"當你需要跨全部頁面搜尋或篩選時，再載入圖譜資料。"
        )
...
def graph_category_options(nodes: list[Mapping[str, Any]]) -> str:
    ...
    return '<option value="all">全部類型</option>' + "".join(
...
def graph_legend_items(colors: Mapping[str, str] = GRAPH_CATEGORY_COLORS) -> str:
    return "".join(
        ...
        f'title="篩選知識圖譜至 {html.escape(str(category), quote=True)}">'
...
(function() {{
  ...
  async function copyGraphLink() {{
    ...
    try {{
      ...
      copyLinkButton.textContent = '複製失敗';
    }}
    window.setTimeout(function() {{ copyLinkButton.textContent = previous || '複製連結'; }}, 1400);
  }}
  ...
  function syncLabelsButton() {{
    ...
    labelsButton.textContent = labelMode === 'all' ? '標籤：全部' : (labelMode === 'neighbors' ? '標籤：局部' : '標籤：精簡');
  ...
  function updateStatus() {{
    ...
    var parts = [
      currentNodes.length + '/' + totalNodeCount + ' 節點',
      currentEdges.length + '/' + totalEdgeCount + ' 邊',
      Math.round(zoom * 100) + '%'
    ];
    if (!fullGraphLoaded) parts.push('快速總覽');
    if (categoryValue !== 'all') parts.push(categoryValue);
    if (sizeMode !== 'category') parts.push('大小 ' + sizeMode);
    if (labelMode !== 'sparse') parts.push('標籤 ' + labelMode);
    if (depthValue !== 'all') parts.push('深度 ' + depthValue);
    if (graphTooLargeForMotion()) parts.push('動態效果已限制');
    if (graphTooLargeForDefaultLabels() && labelMode === 'sparse') parts.push('標籤已精簡');
    if (graphNeedsFastRender(currentNodes, currentEdges)) parts.push('快速繪製');
    if (nodes.length > overviewNodeLimit && currentNodes.length < nodes.length) parts.push('顯示上限 ' + overviewNodeLimit);
    if (fullGraphLoaded && initialGraphMode !== 'full') parts.push('資料已載入');
    if (fullGraphLoading) parts.push('載入圖譜資料中');
    if (searchTerm) {{
      var matches = totalSearchMatches();
      parts.push(matches + ' 筆符合');
      if (matches > SEARCH_LABEL_LIMIT) parts.push('符合項目標籤已限制');
    }}
    var locked = pinnedCount();
    if (locked) parts.push(locked + ' 個已放置');
    if (selectedNode) parts.push('已選取 ' + selectedNode.id);
  ...
  function syncDepthControl() {{
    ...
    depthFilter.title = selectedNode ? '將圖譜限制在所選節點的鄰近範圍。' : '請先選取節點，才能依鄰近範圍篩選。';
  }}
  ...
  function updateInspector() {{
    ...
    if (!selectedNode) {{
      inspectorTitle.textContent = '請選取節點';
      inspectorMeta.textContent = '點一下節點即可檢視。拖曳節點可調整位置。雙擊節點或使用「開啟頁面」即可前往。';
    ...
    inspectorMeta.textContent = selectedNode.category + ' · ' + neighbors.length + ' 個已連結頁面';
    ...
    if (neighbors.length > 10) {{
      ...
      more.textContent = '還有 ' + (neighbors.length - 10) + ' 筆';
  ...
  function setMotionPaused(next) {{
    ...
    if (motionButton) {{
      ...
      motionButton.textContent = graphTooLargeForMotion() ? '動態效果已限制' : (motionPaused ? '動態效果已暫停' : '動態效果開啟');
    }}
    updateStatus();
  ...
  function setFullscreen(next) {{
    ...
    fullscreenButton.textContent = next ? '退出全螢幕' : '全螢幕';
  ...
  function applyGraphPayload(payload) {{
    ...
    if (loadFullButton) {{
      ...
      loadFullButton.textContent = '全部資料已就緒；總覽仍受限';
    }}
  }}
  ...
  function loadFullGraph() {{
    ...
    if (loadFullButton) {{
      loadFullButton.disabled = true;
      loadFullButton.textContent = '載入全部資料中...';
    }}
    updateStatus();
    fetch('/api/graph')
      ...
      .catch(function() {{
        ...
        if (loadFullButton) {{
          ...
          loadFullButton.textContent = '重試載入全部資料';
        }}
        if (status) status.textContent = '圖譜資料載入失敗；本機 API 未回傳圖譜資料。';
      }});
  }}
...
def render_graph_empty_body() -> str:
    ...
    return (
        '<div class="breadcrumb"><a href="/">BrainHub</a> / 知識圖譜</div>'
        "<h1>知識圖譜</h1>"
        '<div class="graph-empty">'
        "<strong>目前還沒有知識圖譜頁面。</strong><br>"
        "請新增來源、匯入後重建反向連結。"
        '<div class="page-actions">'
        '<a class="button-link" href="/ingest">開啟匯入</a>'
        f"{copy_button('ingest the new raw BrainHub files', '複製匯入提示詞')}"
        "</div>"
        "</div>"
    )
    ...
    *,
    ...
    if graph_mode != "full":
        load_full_button = (
            ...
            'title="載入全部圖譜資料以供搜尋、篩選與聚焦鄰近範圍；'
            '在縮小範圍前，畫布仍會維持上限。">'
            f"載入全部資料（{total_node_count} 個節點）</button>"
        )
    focus_html = ""
    state_parts = []
    if focus_label:
        state_parts.append(f'聚焦於 <strong>{html.escape(focus_label)}</strong> · 深度 {html.escape(str(focus_depth))}')
    if search_label:
        state_parts.append(f'搜尋 <strong>{html.escape(search_label)}</strong>')
    if category_label and category_label != "all":
        state_parts.append(f'類型 <strong>{html.escape(category_label)}</strong>')
    if size_label and size_label != "category":
        state_parts.append(f'大小 <strong>{html.escape(size_label)}</strong>')
    if label_label and label_label != "sparse":
        state_parts.append(f'標籤 <strong>{html.escape(label_label)}</strong>')
    if state_parts:
        focus_html = (
    ...
    if graph_mode != "full":
        bounded_html = (
            '<p class="graph-focus-note">'
            "<strong>有限總覽：</strong>"
            f"目前顯示 {html.escape(str(node_count))} / {html.escape(str(total_node_count))} 個節點。"
            "請先使用搜尋、類型篩選、聚焦鄰近範圍，或載入全部資料，再要求瀏覽器繪製整個"
            "知識圖譜。"
            "</p>"
        )
    ...
    return (
        '<div class="breadcrumb"><a href="/">BrainHub</a> / 知識圖譜</div>'
        "<h1>知識圖譜</h1>"
        '<p class="meta">面對大型 wiki，請善用全螢幕、縮放、平移與精簡標籤。'
        "知識圖譜是用來探索鄰近範圍，而不是一次讀完每個標籤。"
        f"{html.escape(graph_note)}</p>"
        f"{bounded_html}"
        f"{focus_html}"
        '<section id="graph-frame" class="graph-frame">'
        '<div class="graph-toolbar" aria-label="知識圖譜控制項">'
        '<button id="graph-reset" type="button">重設</button>'
        '<button id="graph-fit" type="button">置中顯示</button>'
        '<button id="graph-labels" type="button" aria-pressed="false">標籤</button>'
        '<button id="graph-motion" type="button" aria-pressed="false">動態效果</button>'
        '<button id="graph-fullscreen" type="button" aria-pressed="false">全螢幕</button>'
        '<button id="graph-copy-link" type="button">複製連結</button>'
        f"{load_full_button}"
        '<label class="graph-control">搜尋節點'
        '<input id="graph-search" type="search" placeholder="節點標題"></label>'
        '<label class="graph-control">類型'
        f'<select id="graph-category">{category_options}</select></label>'
        '<label class="graph-control">大小'
        '<select id="graph-size"><option value="category">依類型</option>'
        '<option value="degree">依連結數</option></select></label>'
        '<label class="graph-control">標籤'
        '<select id="graph-label-density"><option value="sparse">精簡</option>'
        '<option value="neighbors">鄰近節點</option><option value="all">全部</option></select></label>'
        '<label class="graph-control">顯示筆數'
        '<select id="graph-display-limit"><option value="250">250 個節點</option>'
        '<option value="650" selected>650 個節點</option><option value="1000">1000 個節點</option></select></label>'
        '<label class="graph-control">鄰近範圍'
        '<select id="graph-depth"><option value="all">全部</option><option value="1">1 跳</option>'
        '<option value="2">2 跳</option><option value="3">3 跳</option></select></label>'
        '<span id="graph-status" class="graph-status" aria-live="polite">'
        f"{node_count}/{total_node_count} 節點 · {edge_count}/{total_edge_count} 邊</span>"
        "</div>"
        '<div class="graph-shell">'
        '<canvas id="graph-canvas" tabindex="0" role="img" '
        f'aria-label="知識圖譜，共 {node_count} 個節點與 {edge_count} 條邊"></canvas>'
        '<aside id="graph-inspector" class="graph-inspector" aria-live="polite">'
        '<strong id="graph-inspector-title">請選取節點</strong>'
        '<p id="graph-inspector-meta">點一下節點即可檢視。拖曳節點可調整位置。'
        "雙擊節點或使用「開啟頁面」即可前往。</p>"
        '<div id="graph-inspector-links" class="graph-inspector-links"></div>'
        '<button id="graph-focus" type="button" disabled>聚焦鄰近範圍</button>'
        '<button id="graph-local" type="button" disabled>開啟局部圖譜</button>'
        '<button id="graph-open" type="button" disabled>開啟頁面</button>'
        "</aside>"
        "</div>"
