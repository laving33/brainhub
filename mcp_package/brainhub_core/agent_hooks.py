"""Agent session-hook configuration helpers for BrainHub.

Hooks let supported agents run the BrainHub memory loop automatically:
...
_HOOK_SCRIPT_MARKER = "brainhub_engine.py"
...
class AgentHookConfig:
    ...
    restart_hint: str = "Restart the agent; new sessions will start with the BrainHub memory brief."
...
HOOK_AGENT_CONFIGS: tuple[AgentHookConfig, ...] = (
    ...
    AgentHookConfig(
        ...
        restart_hint=(
            ...
            "new sessions will then start with the BrainHub memory brief."
        ),
    ),
    AgentHookConfig(
        name="cursor",
...
def hook_supported_agents() -> tuple[str, ...]:
    """Return canonical agent names that support `bh connect --hooks`."""
...
def build_agent_hooks_payload(
    *,
    ...
    behavior = [
        f"{config.start_event}: injects a bounded BrainHub memory brief into new agent sessions.",
    ]
    if config.end_event:
        behavior.append(
        ...
        behavior.append(
            f"{config.display_name} has no session-end hook event; end sessions with `bh session-end` "
...
def extract_transcript_text(
    ...
    *,
    ...
    tool calls/results and meta entries, and returns the most recent messages
    within `max_chars`. Pass roles=("user",) to mine only what the user said —
    memory proposals should come from the user's own words, not the assistant's
    prose, which would otherwise be mis-attributed as user preferences.
    """
    role_set = set(roles)
    try:
