"""Local proof signals for BrainHub memory value."""
...
def _link_command(*parts: object, project: str = "") -> str:
    command = ["bh", *(str(part) for part in parts if str(part) != "")]
    if project:
...
def memory_wins_payload(
    ...
    *,
    ...
    """Summarize local, non-telemetry signals that BrainHub memory is carrying value."""
    ...
    wins = [
        ...
        _win(
            ...
            "what does BrainHub remember about me?",
        ),
        _win(
            "review_gate",
            "Review gate",
            ...
            "review BrainHub memory inbox",
        ),
        _win(
            "source_grounding",
            "Provenance present",
            len(source_backed),
            "Memories with source metadata make it easier to inspect why BrainHub knows something.",
            "explain the most important BrainHub memory",
        ),
        _win(
            "project_continuity",
            "Project continuity",
            ...
            "brief me from BrainHub for this project",
        ...
        _win(
            ...
            "audit BrainHub memory",
        ),
        _win(
            "recent_changes",
            "Recent memory changes",
            len(updated_records),
            "Updated memories show where BrainHub has evolved rather than staying as a one-time demo.",
            "what changed in BrainHub memory recently?",
        ),
    ]
    return {
        "schema": "brainhub-memory-wins-v1",
        ...
        "honest_note": (
            ...
            "BrainHub does not track whether an agent used a memory."
        ),
    }
        ...
        return [
            {
                ...
                "reason": "A wins report becomes useful after BrainHub has at least one durable memory.",
